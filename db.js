...
import { pacificDateString, subtractDays } from './dates.js';
...
export async function ensurePoolTopped(env, db, email, today) {
  ...
  const stmt = db.prepare(
    'INSERT OR IGNORE INTO word_pool (user_email, text, mandarin, status, created_date) VALUES (?, ?, ?, ?, ?)'
  );
  const inserts = batch.map((item) => stmt.bind(email, item.text, item.mandarin, 'untested', today));
...
export async function getOrCreateTodayWords(db, email, today) {
  const { results: existing } = await db
    .prepare(
      `SELECT dl.id as logId, dl.word_id as wordId, dl.order_index as orderIndex,
              ...
              wp.text as text, wp.mandarin as mandarin
  ...
  // Top up today's set if it's short of DAILY_WORD_COUNT -- either because
  // this is the first visit today (existing.length === 0), or because
  // DAILY_WORD_COUNT was raised after today's set was already created and
  // we want the increase to take effect immediately rather than tomorrow.
  const needed = CONFIG.DAILY_WORD_COUNT - existing.length;
  if (needed <= 0) return existing;

  const cooldownCutoff = subtractDays(today, CONFIG.PASS_COOLDOWN_DAYS);
  // Never re-select a word already in today's set.
  const usedIds = existing.map((w) => w.wordId);
  const exclusion = usedIds.length ? `AND id NOT IN (${usedIds.map(() => '?').join(',')})` : '';

  const { results: failedWords } = await db
    .prepare(
      `SELECT id, text, mandarin FROM word_pool
       WHERE user_email = ? AND status = 'failed' ${exclusion}
       ORDER BY last_result_date ASC
       LIMIT ?`
    )
    .bind(email, ...usedIds, needed)
    .all();

  const remaining = needed - failedWords.length;
  let fillerWords = [];
  if (remaining > 0) {
    const { results } = await db
      .prepare(
      ...
      .bind(email, cooldownCutoff, ...usedIds, remaining)
      .all();
    fillerWords = results;
  }
  ...
  await db.batch(chosen.map((w, i) => insertStmt.bind(email, today, w.id, startIndex + i)));
...
export async function getHistory(db, email) {
  const { results } = await db
    .prepare(
      `SELECT date,
         SUM(CASE WHEN final_status = 'correct' THEN 1 ELSE 0 END) as correct,
