...
- **Frontend** (`frontend/`): static HTML/CSS/JS, meant to be served from
...
Create a free-tier "Speech" resource in the Azure Portal
(portal.azure.com → Create a resource → AI + Machine Learning → Speech
service → pricing tier F0, free). Grab its key and region from the
resource's "Keys and Endpoint" page — those are the two secrets above.

### 3. Frontend

Edit `frontend/app.js` and set `WORKER_URL` to the `*.workers.dev` URL
from step 1. Then push `frontend/` to a GitHub repo and enable GitHub
Pages (Settings → Pages → deploy from branch, root or `/frontend` as the
source folder).
